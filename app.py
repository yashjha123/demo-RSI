...
import diskcache
cache = diskcache.Cache("./cache456")
long_callback_manager = DiskcacheLongCallbackManager(cache)
from dash_extensions.enrich import DashProxy, ServersideOutputTransform


# output_defaults = dict(backend=FileSystemStore(
#     cache_dir="./cache123"), session_check=True)
# app = dash.Dash(__name__, background_callback_manager=background_callback_manager)
app = DashProxy(__name__, external_stylesheets=[dbc.themes.DARKLY], suppress_callback_exceptions=True,long_callback_manager=long_callback_manager,transforms=[ServersideOutputTransform()])
